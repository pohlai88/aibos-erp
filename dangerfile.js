...
const migrations = danger.git.modified_files.filter(file => 
...
const packageJsonChanges = danger.git.modified_files.filter(file => 
...
const tsFiles = danger.git.modified_files.filter(file => 
...
const docFiles = danger.git.modified_files.filter(file => 
...
const breakingChanges = danger.git.commits.some(commit => 
...
const invalidCommits = danger.git.commits.filter(commit => {
  ...
  return !message.match(/^(feat|fix|docs|style|refactor|test|chore|perf|ci|build|revert)(\(.+\))?: .+/);
});
...
const envFiles = danger.git.modified_files.filter(file => 
...
if (danger.git.modified_files.length > 0) {
  message(`✅ Danger.js checks completed for ${danger.git.modified_files.length} modified files`);
}
